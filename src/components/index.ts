//utills
export { Btn } from '@/components/utils/Btn'


//Slides 
export { HomeSlide } from '@/components/slides/Home'
export { AboutSlide } from '@/components/slides/About'
export { ProjectsSlide } from '@/components/slides/Projects'
export { ContactSlide } from '@/components/slides/Contact'
export { DjosSlide } from '@/components/slides/Djos'
export { SkillsSlide } from '@/components/slides/Skills'
export { ContainerSlides } from '@/components/slides/ContainerSlides'

//Geometries 
export { ContainersModel } from '@/components/3d/models/ContainersModel'
export { default as Box } from '@/components/3d/models/BoxGeometry'
export { default as Cone } from '@/components/3d/models/ConeGeometry'
export { default as HomeGeometry } from '@/components/3d/models/HomeGeometry'
export { default as Lathe } from '@/components/3d/models/LatheGeometry'
export { default as Octahedron } from '@/components/3d/models/OctahedronGeometry'
export { default as Poly } from '@/components/3d/models/PolyGeometry'
export { default as Ring } from '@/components/3d/models/RingGeometry'
export { default as Torus } from '@/components/3d/models/TorusGeometry'
export { default as TorusKnots } from '@/components/3d/models/TorusKnot'

/*



*/